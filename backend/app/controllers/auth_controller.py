...
def register():
    ...
    field_error = registration_error(full_name, email, password, phone, country)
    if field_error:
    ...
    try:
        ...
        # above; the unique index is the real guard, so report it identically.
        db.session.rollback()
        return jsonify({"message": "Email already registered"}), 400

    # Best-effort welcome email: send_email never raises, so a mail outage
    # cannot fail a registration that is already committed.
