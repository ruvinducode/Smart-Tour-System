...
from app.services.vehicle_matching_service import vehicle_ids_for_type
...
def get_tour_details(tour_id):
    try:
        ...
        # A driver can view a tour's details either once they're the assigned
        # driver, OR while it's still an open, unclaimed request matching
        # their vehicle type — mirroring the exact same eligibility check
        # get_driver_tour_requests() uses to decide what a driver sees in
        # their request list. Without the second condition, a driver could
        # never open a tour's details before accepting it, since no Booking
        # row exists yet at that stage.
        is_eligible_driver = False
        if claims.get("role") == "driver":
            ...
            if existing_booking and str(existing_booking.driver_id) == str(current_id):
                is_eligible_driver = True
            elif not existing_booking or existing_booking.driver_id is None:
                driver = Driver.query.get(current_id)
                if driver and tour.status != "cancelled":
                    matching_vehicle_ids = vehicle_ids_for_type(driver.vehicle_type)
                    is_eligible_driver = tour.vehicle_id in matching_vehicle_ids

        if not (is_admin or is_owner or is_eligible_driver):
...
def update_driver_location(tour_id):
    ...
    if new_heading is not None:
        try:
            new_heading = float(new_heading) % 360
        except (TypeError, ValueError):
            new_heading = None
    ...
    if new_heading is not None:
        tour.driver_heading = new_heading
